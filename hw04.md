...
| 2007 | 80.653 | 72.96100 |
  ...
  - Then I’ll do it for different countries, eg the countries of a
    continent
...
```
  ...
  - From the above experiment, we can see that full-join() with reversed
    pass in generated the same result. The reason is that full-join
    function shows all rows and columns from both x and y.
...
```
  ...
  - From the above experiment we can see that changing the order of
    passing in parameters does not change the result. Because the
    inner\_join() only find the countries appeard in both data frames.
...
| Costa Rica               | CRI        |      188 |
  ...
  - We can see that this time the results is different. Though
    semi\_join() returns the overlapped countries, the results depend on
    the first parameter.
...
| Algeria     | DZA        |       12 | Africa    | 1977 |  58.014 | 17152804 | 4910.4168 |
  ...
  - The results depends on the countries of the first parameter. if
    ...
    countries only exist in gapminder. And for the countries only
    existed in country\_codes, the value for that in gaominder is set to
    NA.
...
| Netherlands Antilles  | ANT        |      530 |
  ...
  - anti\_join() shows the rows in the first parameter that does not
    exist in the second one. Also the columns in the result is dependent
